...
// Main
void main() async {
  LaravelJsonApiAdapter api =
      LaravelJsonApiAdapter('maeth.herokuapp.com', '/api/v1');
  ...
  var response = await _saveArticle(adapter, newArticle.jsonApiDoc);
...
Future<User> _getUser(Adapter adapter) async {
  return User(await adapter.find(
      ...
      queryParams: {'include': 'articles'}) as LaravelJsonApiDocument);
}

Future<Iterable<Category>> _getCategories(Adapter adapter) async {
  return (await adapter.findAll('categories'))
      .map<Category>((category) => Category(category as LaravelJsonApiDocument))
      .toList();
}

Future _saveArticle(Adapter adapter, Object model) async {
  try {
    Article(await adapter.save('articles', model) as LaravelJsonApiDocument);
    return true;
  } catch (e) {
    print(e);
    Article temp = Article(model as LaravelJsonApiDocument);
